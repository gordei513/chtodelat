...
def rabin_karp(text, pattern):
    ...
    # Менять отсюда =) ---- vvvvv ----
    k = ""
    h_sub = sum(ord(c) for c in pattern) 
    h = sum(ord(c) for c in text[:len(pattern)])
    for pos in range(0,  len(text)- len(pattern)):
        k = pattern
        if h != h_sub:  
              h = h - ord(text[pos]) + ord(text[pos + len(pattern)]) #,,сдвиг и проверка,,
        h = h - ord(text[pos]) + ord(text[pos + len(pattern)]) 
        flag = True
    ...
    if k != "":        
      if text[len(text)-len(pattern):len(text)] == pattern:
